...
namespace integrals {
    ...
    template<class F>
    double calculate_riemann_sum_interval(F function, double x_start, double delta_x, double y_start,
                                 double delta_y, size_t steps_x, size_t steps_y) {
        double result = 0;
        for (size_t step_x = 0; step_x != steps_x; step_x++) {
            for (size_t step_y = 0; step_y != steps_y; step_y++) {
                auto value_of_func = function(x_start + (double) step_x * delta_x,
                                              y_start + (double) step_y * delta_y);
                result += value_of_func;
            }
        }
        return result;
    ...
    void consume(const F &function, thread_safe_queue<Interval> &input_deque, thread_safe_queue<double> &output_deque) {
        while (true) {
            ...
            auto res = calculate_riemann_sum_interval<F>(function, interval.x0, interval.dx, interval.y0, interval.dy, interval.Nx,
                                             interval.Ny);
            output_deque.push(res);
        }
    }

    /*
     * Computes an integral using thread-safe deque. The main thread is
     * producer (produces intervals to integrate over) and other threads are
     * consumers (compute integrals over intervals acquired from deque)
     * */
    template<class F>
    std::tuple<double, double, double>
    calculate_integral_parallel_queue(size_t thread_count, size_t pts_per_interval, F function, double abs_err,
                                      double rel_err,
                                      double x_start, double x_end, double y_start,
                                      double y_end, size_t init_steps_x,
                                      size_t init_steps_y, size_t max_iter) {
        thread_safe_queue<Interval> input_deque;
        thread_safe_queue<double> output_deque;
        ...
        do {
            ...
            // adding up results while deque isn't empty
            for (size_t i {0}; i != num_of_intervals; i++) {
                second_riemann_sum += output_deque.pop();
            }
